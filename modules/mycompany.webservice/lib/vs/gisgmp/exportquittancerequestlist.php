...
/**Список запросов на экспорт квитанций
 * Class ExportQuittanceRequestList
 * @package MyCompany\WebService\VS\Gisgmp
 */
class ExportQuittanceRequestList
{
    const IBLOCK_CODE = 'gis-gmp-export-quittance-request';
    public array $items;
    ...
    public static function addItem($params): array
    {
        ...
        $fields = [
            'IBLOCK_ID' => RestHelper\Iblock::getIblockIdByCode(ExportQuittanceRequestList::IBLOCK_CODE),
        ...
        $object = new ExportQuittanceRequestList();
    ...
    {
        $preparedProps = [];
        $iblockid = RestHelper\Iblock::getIblockIdByCode(self::IBLOCK_CODE);
        $dbProps = RestHelper\Iblock::getIblockProperties($iblockid);
        $enumValue = Helper::getPropertyEnumValue($iblockid, 'IS_SELECT_END', $input["isselectend"]);
        $schema = (RestHelper\Options::getMapFromOptions('schema_exportquittancerequestlist'));
        foreach ($input as $propItemKey => $propItem) {
            if (!empty($dbProps[$schema[$propItemKey]]) && $dbProps[$schema[$propItemKey]]['PROPERTY_TYPE'] == 'L') {
                $preparedProps[$schema[$propItemKey]] = $enumValue;
            } else {
    ...
    public static function updateItem(array $params, int $elementId): array
    {
        ...
        $props = self::prepareProps($params);
        $fields = [
            'IBLOCK_ID' => RestHelper\Iblock::getIblockIdByCode(self::IBLOCK_CODE),
            'NAME' => $params["name"]
        ];
